...
import io.thestencil.workflows.handlers.WorkflowsResourceHandler;
...
@Recorder
public class WorkflowsRecorder {
  public static final String FEATURE_BUILD_ITEM = "stencil-workflows";
  ...
  public BeanContainerListener configureBuildtimeConfig(
    ...
    return beanContainer -> {
      WorkflowsProducer producer = beanContainer.instance(WorkflowsProducer.class);
      producer
  ...
  public void configureRuntimeConfig(RuntimeConfig runtimeConfig) {
    CDI.current().select(WorkflowsProducer.class).get().setRuntimeConfig(runtimeConfig);
  }
  ...
  public Handler<RoutingContext> userActionsHandler() {
    ...
    return new WorkflowsResourceHandler(association, currentVertxRequest);
  }
